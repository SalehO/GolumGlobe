...
class Board:
    ...
    def reset(self,golemIndexes=[]):
        self.currentIndex = self.initalIndex

        for i in golemIndexes:
            self.board[i].updateTileType("GOLEM")
    ...
    def attack(self,index):
        print(" attacking {} {}".format(index,self.board[index].attack()))
        return self.board[index].attack()

    def getObservationForIndex(self,index=-1,successfulAttack=False):
        ...
        if successfulAttack:
           obs_type.append("Killed") 

        return Observation(obs_type)
